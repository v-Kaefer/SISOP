import math
import time
import threading
from enum import Enum
from collections import deque
import queue
import os
from datetime import datetime
import configparser

# -------------------------------------------------------------------------------------------------------
# --------------------- H A R D W A R E - Definições de HW
# -------------------------------------------------------------------------------------------------------

class Opcode(Enum):
    DATA, ___, JMP, JMPI, JMPIG, JMPIL, JMPIE, JMPIM, JMPIGM, JMPILM, JMPIEM, JMPIGK, JMPILK, JMPIEK, JMPIGT, ADDI, SUBI, ADD, SUB, MULT, LDI, LDD, STD, LDX, STX, MOVE, SYSCALL, STOP = range(28)

class Interrupts(Enum):
    NO_INTERRUPT, INT_ENDERECO_INVALIDO, INT_INSTRUCAO_INVALIDA, INT_OVERFLOW, PAGE_FAULT, IO_COMPLETION = range(6)

class Word:
    def __init__(self, opc, ra, rb, p): self.opc, self.ra, self.rb, self.p = opc, ra, rb, p

class Memory:
    def __init__(self, size): self.data = [Word(Opcode.___, -1, -1, -1) for _ in range(size)]

class CPU(threading.Thread):
    def __init__(self, memory, so, debug=False):
        super().__init__(name="CPU")
        self.max_int, self.min_int = 32767, -32767
        self.memory, self.reg, self.debug, self.so = memory.data, [0] * 10, debug, so
        self.pc, self.ir, self.running_process, self.instructions_executed = 0, None, None, 0
        self.interrupt_register, self.interrupt_data = Interrupts.NO_INTERRUPT, None
        self.interrupt_lock, self.cpu_has_process_event = threading.Lock(), threading.Event()
        self.gm = None

    def set_context(self, pcb):
        # Zera a contagem de instrucoes ao carregar um novo processo
        self.running_process, self.pc, self.reg, self.instructions_executed = pcb, pcb.pc, pcb.registers.copy(), 0 
        pcb.state = PCB.ProcessState.RUNNING

    def set_gerente_memoria(self, gm):
        self.gm = gm

    def _translate_address(self, logical_address, is_write_op=False):
        if self.running_process is None: self.so.interrupt_handler.handle(Interrupts.INT_ENDERECO_INVALIDO, logical_address); return -1
        tam_pg = self.gm.get_tam_pg()
        page_index = logical_address // tam_pg
        if not (0 <= page_index < len(self.running_process.page_table)): self.so.interrupt_handler.handle(Interrupts.INT_ENDERECO_INVALIDO, logical_address); return -1
        page_entry = self.running_process.page_table[page_index]
        if not page_entry.valid_bit: self.so.interrupt_handler.handle(Interrupts.PAGE_FAULT, logical_address); return -1
        if is_write_op: page_entry.dirty_bit = True
        return (page_entry.frame_number * tam_pg) + (logical_address % tam_pg)

    def _execute_one_instruction(self):
        physical_pc = self._translate_address(self.pc)
        if physical_pc == -1: return True
        self.ir = self.memory[physical_pc]
        if self.debug: print(f"    [CPU] PC: {self.pc} (Proc: {self.running_process.id}) -> INSTR: ", end=""); self.so.utils.dump_word(self.ir)
        
        opc, ra, rb, p = self.ir.opc, self.ir.ra, self.ir.rb, self.ir.p
        
        stop_quantum = False
        
        if opc == Opcode.SYSCALL: self.so.syscall_handler.handle(); stop_quantum = True
        elif opc == Opcode.STOP: self.so.syscall_handler.stop(); stop_quantum = True
        else:
            addr = -1
            is_write = opc in [Opcode.STD, Opcode.STX]
            if opc == Opcode.STD: addr = self._translate_address(p, is_write)
            elif opc == Opcode.STX: addr = self._translate_address(self.reg[ra], is_write)
            elif opc == Opcode.LDD: addr = self._translate_address(p)
            elif opc == Opcode.LDX: addr = self._translate_address(self.reg[rb])
            elif opc == Opcode.JMPIM: addr = self._translate_address(p)
            elif opc in [Opcode.JMPIGM, Opcode.JMPILM, Opcode.JMPIEM]: addr = self._translate_address(p)

            if addr == -1 and opc in [Opcode.STD, Opcode.STX, Opcode.LDD, Opcode.LDX, Opcode.JMPIM, Opcode.JMPIGM, Opcode.JMPILM, Opcode.JMPIEM]:
                return True

            if opc == Opcode.LDI: self.reg[ra] = p; self.pc += 1
            elif opc == Opcode.LDD: self.reg[ra] = self.memory[addr].p; self.pc += 1
            elif opc == Opcode.LDX: self.reg[ra] = self.memory[addr].p; self.pc += 1
            elif opc == Opcode.STD: self.memory[addr] = Word(Opcode.DATA, -1, -1, self.reg[ra]); self.pc += 1
            elif opc == Opcode.STX: self.memory[addr] = Word(Opcode.DATA, -1, -1, self.reg[rb]); self.pc += 1
            elif opc == Opcode.MOVE: self.reg[ra] = self.reg[rb]; self.pc += 1
            elif opc in [Opcode.ADD, Opcode.ADDI, Opcode.SUB, Opcode.SUBI, Opcode.MULT]:
                val = self.reg[rb] if opc in [Opcode.ADD, Opcode.SUB, Opcode.MULT] else p
                if opc in [Opcode.ADD, Opcode.ADDI]: self.reg[ra] += val
                elif opc in [Opcode.SUB, Opcode.SUBI]: self.reg[ra] -= val
                elif opc == Opcode.MULT: self.reg[ra] *= val
                self.pc += 1
            elif opc == Opcode.JMP: self.pc = p
            elif opc == Opcode.JMPIM: self.pc = self.memory[addr].p
            elif opc == Opcode.JMPIG and self.reg[rb]>0: self.pc=self.reg[ra]
            elif opc == Opcode.JMPIL and self.reg[rb]<0: self.pc=self.reg[ra]
            elif opc == Opcode.JMPIE and self.reg[rb]==0: self.pc=self.reg[ra]
            elif opc == Opcode.JMPIGK and self.reg[rb]>0: self.pc=p
            elif opc == Opcode.JMPILK and self.reg[rb]<0: self.pc=p
            elif opc == Opcode.JMPIEK and self.reg[rb]==0: self.pc=p
            elif opc == Opcode.JMPIGT and self.reg[ra]>self.reg[rb]: self.pc=p
            elif opc == Opcode.JMPIGM and self.reg[rb]>0: self.pc=self.memory[addr].p
            elif opc == Opcode.JMPILM and self.reg[rb]<0: self.pc=self.memory[addr].p
            elif opc == Opcode.JMPIEM and self.reg[rb]==0: self.pc=self.memory[addr].p
            else: self.pc += 1
        
        self.instructions_executed += 1
        return stop_quantum

    def run(self):
        while not self.so.shutdown:
            # 1. Trata interrupções pendentes. Esta é a maior prioridade.
            with self.interrupt_lock:
                if self.interrupt_register != Interrupts.NO_INTERRUPT:
                    self.so.interrupt_handler.handle(self.interrupt_register, self.interrupt_data)
                    self.interrupt_register, self.interrupt_data = Interrupts.NO_INTERRUPT, None
                    if self.running_process and self.running_process.state != PCB.ProcessState.RUNNING:
                        self.running_process = None

            # 2. Se a CPU está livre, tenta escalar um novo processo.
            if self.running_process is None:
                self.so.scheduler.schedule()

            # 3. Se um processo foi escalonado, executa seu quantum.
            if self.running_process:
                quantum_end = False
                while not quantum_end and self.instructions_executed < self.so.quantum:
                    quantum_end = self._execute_one_instruction()
                    # Uma interrupção externa (como I/O) pode preemptar o processo.
                    with self.interrupt_lock:
                        if self.interrupt_register != Interrupts.NO_INTERRUPT:
                            quantum_end = True

                # 4. Após o quantum, trata o processo que estava em execução.
                if self.running_process:
                    if self.running_process.state == PCB.ProcessState.RUNNING:
                        self.so.scheduler.handle_quantum_end(self.running_process)
                    # Se foi bloqueado/finalizado, o handler já tratou. A CPU fica livre.
                    self.running_process = None
            else:
                # 5. Se não há processo para rodar, a CPU espera por um evento.
                self.cpu_has_process_event.wait(timeout=1.0)
                if self.so.shutdown: break
                self.cpu_has_process_event.clear()

class IODevice(threading.Thread):
    def __init__(self, so):
        super().__init__(name="IODevice")
        self.so, self.request_queue, self.simulated_io_time = so, queue.Queue(), 2.0
    def submit_request(self, pcb, op): self.request_queue.put((pcb, op))
    def run(self):
        while not self.so.shutdown:
            try:
                pcb, op = self.request_queue.get(timeout=1)
                print(f"    [IO_DEVICE] Iniciando I/O para o processo {pcb.id}")
                time.sleep(self.simulated_io_time)
                print(f"    [IO_DEVICE] I/O para o processo {pcb.id} concluído.")
                with self.so.hw.cpu.interrupt_lock:
                    self.so.hw.cpu.interrupt_register = Interrupts.IO_COMPLETION
                    self.so.hw.cpu.interrupt_data = pcb
                self.so.hw.cpu.cpu_has_process_event.set() # Acorda a CPU para tratar a interrupcao
            except queue.Empty: continue

class HW:
    def __init__(self, mem_size, so_ref):
        self.mem = Memory(mem_size)
        self.cpu = CPU(self.mem, so_ref, debug=False)

# -------------------------------------------------------------------------------------------------------
# --------------------- SW - Sistema Operacional
# -------------------------------------------------------------------------------------------------------
class PageTableEntry:
    def __init__(self, page_idx): 
        self.frame_number, self.valid_bit, self.dirty_bit, self.on_swap, self.page_idx = -1, False, False, False, page_idx
    
    def get_where_flag(self):
        if self.valid_bit:
            return "mp"
        elif self.on_swap:
            return "ms"
        else:
            return "_"

class PCB:
    _next_id = 0
    class ProcessState(Enum): NEW, READY, RUNNING, BLOCKED, FINISHED = range(5)
    def __init__(self, program, num_pages):
        self.id = PCB._next_id; PCB._next_id += 1
        self.program, self.pc, self.registers, self.state, self.block_reason = program, 0, [0] * 10, PCB.ProcessState.READY, ""
        self.page_table = [PageTableEntry(i) for i in range(num_pages)]
    
    def get_page_table_string(self):
        entries = []
        for entry in self.page_table:
            frame_str = str(entry.frame_number) if entry.valid_bit else "_"
            where = entry.get_where_flag()
            entries.append(f"[{entry.page_idx},{frame_str},{where}]")
        return "{ " + ", ".join(entries) + " }"

class SystemLogger:
    def __init__(self, log_file="system_log.txt"):
        self.log_file = log_file
        self.lock = threading.Lock()
        with open(self.log_file, 'w') as f:
            f.write(f"# Log do Sistema - Iniciado em {datetime.now()}\n")
            f.write("# Formato: ID NomeProg Razao EstadoInicial ProximoEstado TabelaDePaginas\n")
            f.write("-" * 120 + "\n")
    
    def log_state_change(self, pcb, razao, estado_inicial, estado_final):
        with self.lock:
            estado_i = estado_inicial.name if isinstance(estado_inicial, PCB.ProcessState) else estado_inicial
            estado_f = estado_final.name if isinstance(estado_final, PCB.ProcessState) else estado_final
            page_table_str = pcb.get_page_table_string()
            log_line = f"{pcb.id:3d}  {pcb.program.name:15s}  {razao:20s}  {estado_i:10s}  {estado_f:10s}  {page_table_str}\n"
            with open(self.log_file, 'a') as f:
                f.write(log_line)
            print(f"[LOG] {log_line.strip()}")


class GerenteDisco:
    def __init__(self, so): 
        self.so, self.swap_area = so, {}
    
    def save_to_swap(self, pcb, page_entry, page_data):
        print(f"    [DISK] Salvando pagina SUJA {page_entry.page_idx} do processo {pcb.id} na area de swap.")
        self.swap_area[(pcb.id, page_entry.page_idx)] = list(page_data)
        page_entry.on_swap = True
    
    def load_page(self, pcb, page_idx):
        page_entry = pcb.page_table[page_idx]
        page_size = self.so.gm.get_tam_pg()
        if page_entry.on_swap: 
            print(f"    [DISK] Carregando pagina {page_idx} do processo {pcb.id} da area de swap.")
            return self.swap_area.get((pcb.id, page_idx))
        else: 
            print(f"    [DISK] Carregando pagina {page_idx} do processo {pcb.id} do programa original.")
            start = page_idx * page_size
            return pcb.program.image[start : start + page_size]


class GerenteMemoria:
    def __init__(self, tam_mem, tam_pg):
        self.tam_mem, self.tam_pg, self.num_frames = tam_mem, tam_pg, tam_mem // tam_pg
        self.free_frames = [True] * self.num_frames
        self.frame_load_order, self.frame_to_pcb_map, self.lock = deque(), {}, threading.Lock()
        self.frame_metadata = [{"pid": None, "page": None, "dirty": False, "last_used": 0} for _ in range(self.num_frames)]
        self.access_counter = 0
    
    def find_free_frame(self): return next((i for i, free in enumerate(self.free_frames) if free), -1)
    
    def find_victim_frame_fifo(self):
        victim_frame = self.frame_load_order.popleft()
        victim_pcb = self.frame_to_pcb_map.pop(victim_frame, None)
        victim_page_entry = None
        if victim_pcb:
            for entry in victim_pcb.page_table:
                if entry.frame_number == victim_frame:
                    entry.valid_bit = False; victim_page_entry = entry
                    print(f"    [MEM] Vitimacao! Frame {victim_frame} (do processo {victim_pcb.id}, pagina {entry.page_idx}) foi escolhido.")
                    break
        self.frame_metadata[victim_frame] = {"pid": None, "page": None, "dirty": False, "last_used": 0}
        return victim_frame, victim_pcb, victim_page_entry
    
    def allocate_frame_for_page(self, pcb, page_entry, frame_number):
        self.free_frames[frame_number] = False
        self.frame_load_order.append(frame_number)
        self.frame_to_pcb_map[frame_number] = pcb
        page_entry.frame_number, page_entry.valid_bit, page_entry.dirty_bit, page_entry.on_swap = frame_number, True, False, False
        self.access_counter += 1
        self.frame_metadata[frame_number] = {
            "pid": pcb.id,
            "page": page_entry.page_idx,
            "dirty": False,
            "last_used": self.access_counter
        }
    
    def update_frame_access(self, frame_number):
        self.access_counter += 1
        self.frame_metadata[frame_number]["last_used"] = self.access_counter

    def get_tam_pg(self):
        return self.tam_pg

class GerenteProcessos:
    def __init__(self, so):
        self.so = so
        self.ready_queue, self.blocked_queue, self.all_processes = deque(), [], []
        self.lock = threading.Lock()
        self.dump_dir = "memory_dumps"
        os.makedirs(self.dump_dir, exist_ok=True)

    def cria_processo(self, programa):
        with self.lock:
            if not programa: print("Erro: Programa nao encontrado."); return -1

            # Calcula o número de páginas necessário considerando o código e os endereços acessados
            max_address = len(programa.image) - 1

            # Verifica todos os endereços que o programa pode acessar
            for word in programa.image:
                if word.opc in [Opcode.STD, Opcode.LDD, Opcode.JMPIM, Opcode.JMPIGM, Opcode.JMPILM, Opcode.JMPIEM]:
                    if word.p > max_address:
                        max_address = word.p

            num_pages = math.ceil((max_address + 1) / self.so.gm.get_tam_pg())
            pcb = PCB(programa, num_pages)
            pcb.state = PCB.ProcessState.BLOCKED
            pcb.block_reason = "Initial Load"
            self.all_processes.append(pcb)
            self.blocked_queue.append(pcb)
            print(f"[KERNEL] Criando processo {pcb.id} ({programa.name}). Aguardando carga inicial da memoria.")
            self.so.logger.log_state_change(pcb, "criacao", "nulo", PCB.ProcessState.BLOCKED)
        self.so.interrupt_handler.load_initial_page(pcb)
        return pcb.id

    def desaloca_processo(self, pcb):
        self.dump_processo_memoria(pcb)
        with self.so.gm.lock:
            for page_entry in pcb.page_table:
                if page_entry.valid_bit:
                    frame = page_entry.frame_number
                    self.so.gm.free_frames[frame] = True
                    if frame in self.so.gm.frame_to_pcb_map: del self.so.gm.frame_to_pcb_map[frame]
                    if frame in self.so.gm.frame_load_order: self.so.gm.frame_load_order.remove(frame)
                    self.so.gm.frame_metadata[frame] = {"pid": None, "page": None, "dirty": False, "last_used": 0}
        self.all_processes.remove(pcb)
        self.so.logger.log_state_change(pcb, "finalizacao", pcb.state, "FINISHED")
        print(f"[KERNEL] Processo {pcb.id} finalizado e memoria liberada.")

    def dump_processo_memoria(self, pcb):
        dump_file = os.path.join(self.dump_dir, f"processo_{pcb.id}_{pcb.program.name}_dump.txt")
        with open(dump_file, 'w') as f:
            f.write(f"# Dump de Memoria - Processo {pcb.id} ({pcb.program.name})\n")
            f.write(f"# Gerado em: {datetime.now()}\n")
            f.write(f"# PC: {pcb.pc}, Estado: {pcb.state.name}\n")
            f.write(f"# Registradores: {pcb.registers}\n")
            f.write("-" * 80 + "\n\n")
            f.write("Tabela de Paginas:\n")
            for entry in pcb.page_table:
                f.write(f"  Pagina {entry.page_idx}: Frame={entry.frame_number if entry.valid_bit else 'N/A'}, ")
                f.write(f"Valido={entry.valid_bit}, Sujo={entry.dirty_bit}, NoSwap={entry.on_swap}, Where={entry.get_where_flag()}\n")
            f.write("\nConteudo da Memoria Fisica (Frames alocados):\n")
            with self.so.gm.lock:
                for entry in pcb.page_table:
                    if entry.valid_bit:
                        frame = entry.frame_number
                        page_size = self.so.gm.get_tam_pg()
                        start_addr = frame * page_size
                        f.write(f"\n--- Frame {frame} (Pagina {entry.page_idx}) ---\n")
                        for i in range(page_size):
                            addr = start_addr + i
                            if addr < len(self.so.hw.mem.data):
                                word = self.so.hw.mem.data[addr]
                                f.write(f"  Fis[{addr:04d}]: {word.opc.name:10s} R1:{word.ra:2d} R2:{word.rb:2d} P:{word.p:4d}\n")
        print(f"[KERNEL] Dump de memoria salvo em: {dump_file}")

    def list_all_processes(self):
        with self.lock:
            print("\n--- LISTA DE PROCESSOS ---")
            if not self.all_processes: print("Nenhum processo no sistema.")
            for p in self.all_processes:
                blocked_reason = f" ({p.block_reason})" if p.state == PCB.ProcessState.BLOCKED else ""
                print(f"  ID: {p.id} | Nome: {p.program.name:<12} | Estado: {p.state.name:<8}{blocked_reason}")
            print("--------------------------------------------------")

    def dump_processo(self, pcb_id):
        with self.lock:
            pcb = self._find_pcb(pcb_id)
            if not pcb: print(f"Erro: Processo com ID {pcb_id} nao encontrado."); return
            print(f"\n--- DUMP Processo ID: {pcb.id} ---")
            print(f"Estado: {pcb.state.name}, PC: {pcb.pc}, Registradores: {pcb.registers}")
            print(f"Tabela de Paginas: {pcb.get_page_table_string()}")

    def _find_pcb(self, pcb_id):
        return next((p for p in self.all_processes if p.id == pcb_id), None)


class Scheduler:
    def __init__(self, so): self.so = so
    def schedule(self):
        if self.so.hw.cpu.running_process or self.so.shutdown: return
        with self.so.gp.lock:
            if self.so.gp.ready_queue:
                pcb = self.so.gp.ready_queue.popleft()
                print(f"    [ESCALONADOR] Processo {pcb.id} escalonado para a CPU.")
                self.so.logger.log_state_change(pcb, "escalonamento", PCB.ProcessState.READY, PCB.ProcessState.RUNNING)
                self.so.hw.cpu.set_context(pcb)
                self.so.hw.cpu.cpu_has_process_event.set()
    def handle_quantum_end(self, pcb):
        with self.so.gp.lock:
            if pcb.state == PCB.ProcessState.RUNNING:
                print(f"    [ESCALONADOR] Quantum do processo {pcb.id} terminou.")
                pcb.state, pcb.pc, pcb.registers = PCB.ProcessState.READY, self.so.hw.cpu.pc, self.so.hw.cpu.reg.copy()
                self.so.gp.ready_queue.append(pcb)
                self.so.logger.log_state_change(pcb, "preempcao_quantum", PCB.ProcessState.RUNNING, PCB.ProcessState.READY)

class DiskThread(threading.Thread):
    """Thread dedicada para operações de disco (carregamento/salvamento de páginas)"""
    def __init__(self, so):
        super().__init__(name="DiskThread")
        self.so = so
        self.request_queue = queue.Queue()
        self.simulated_disk_time = 0.3

    def submit_request(self, request_type, pcb, page_idx, frame=None, page_data=None):
        """Submete requisição de disco: 'LOAD' ou 'SAVE'"""
        self.request_queue.put((request_type, pcb, page_idx, frame, page_data))

    def run(self):
        while not self.so.shutdown:
            try:
                request_type, pcb, page_idx, frame, page_data = self.request_queue.get(timeout=1)

                if request_type == "SAVE":
                    print(f"    [DISK_THREAD] Salvando pagina {page_idx} do processo {pcb.id} no disco...")
                    time.sleep(self.simulated_disk_time)
                    page_entry = pcb.page_table[page_idx]
                    self.so.disk.save_to_swap(pcb, page_entry, page_data)
                    print(f"    [DISK_THREAD] Pagina {page_idx} do processo {pcb.id} salva com sucesso.")

                elif request_type == "LOAD":
                    print(f"    [DISK_THREAD] Carregando pagina {page_idx} do processo {pcb.id} do disco...")
                    time.sleep(self.simulated_disk_time)
                    page_data = self.so.disk.load_page(pcb, page_idx)

                    # Escreve na memória física (DMA simulado)
                    with self.so.gm.lock:
                        page_size = self.so.gm.get_tam_pg()
                        frame_addr = frame * page_size
                        for i in range(len(page_data)):
                            self.so.hw.mem.data[frame_addr + i] = page_data[i]
                        self.so.gm.allocate_frame_for_page(pcb, pcb.page_table[page_idx], frame)

                    print(f"    [DISK_THREAD] Pagina {page_idx} do processo {pcb.id} carregada no frame {frame}.")

                    # Gera interrupção de conclusão
                    with self.so.hw.cpu.interrupt_lock:
                        self.so.hw.cpu.interrupt_register = Interrupts.IO_COMPLETION
                        self.so.hw.cpu.interrupt_data = (pcb, "PAGE_LOAD")
                    self.so.hw.cpu.cpu_has_process_event.set()

            except queue.Empty:
                continue

class InterruptHandling:
    def __init__(self, so): self.so = so

    def handle(self, interrupt, data):
        if interrupt == Interrupts.IO_COMPLETION:
            # data é uma tupla (pcb, tipo)
            if isinstance(data, tuple):
                pcb, io_type = data
            else:
                pcb, io_type = data, "IO"

            if not pcb: return
            print(f"    [KERNEL] Interrupcao! Tipo: {interrupt.name}, Processo: {pcb.id}, IO_Type: {io_type}")

            with self.so.gp.lock:
                if pcb in self.so.gp.blocked_queue:
                    is_initial_load = pcb.block_reason == "Initial Load"
                    self.so.gp.blocked_queue.remove(pcb)
                    pcb.state, pcb.block_reason = PCB.ProcessState.READY, ""
                    self.so.gp.ready_queue.append(pcb)

                    if io_type == "PAGE_LOAD":
                        razao = "carga_inicial" if is_initial_load else "retorno_page_fault"
                        self.so.logger.log_state_change(pcb, razao, PCB.ProcessState.BLOCKED, PCB.ProcessState.READY)
                        if is_initial_load:
                            print(f"[KERNEL] Processo {pcb.id} pronto e adicionado a fila de prontos.")
                    else:
                        self.so.logger.log_state_change(pcb, "retorno_io", PCB.ProcessState.BLOCKED, PCB.ProcessState.READY)

                    self.so.hw.cpu.cpu_has_process_event.set()
        else:
            pcb = self.so.hw.cpu.running_process
            if not pcb: return
            print(f"    [KERNEL] Interrupcao! Tipo: {interrupt.name}, Processo: {pcb.id}")

            with self.so.gp.lock:
                if interrupt == Interrupts.PAGE_FAULT:
                    if pcb.state == PCB.ProcessState.RUNNING:
                        pcb.state, pcb.block_reason = PCB.ProcessState.BLOCKED, "Page Fault"
                        pcb.pc = self.so.hw.cpu.pc
                        pcb.registers = self.so.hw.cpu.reg.copy()
                        self.so.logger.log_state_change(pcb, "page_fault", PCB.ProcessState.RUNNING, PCB.ProcessState.BLOCKED)
                    if pcb not in self.so.gp.blocked_queue:
                        self.so.gp.blocked_queue.append(pcb)
                else:
                    # Interrupção fatal
                    print(f"    [KERNEL] Interrupcao fatal para o processo {pcb.id}. Removendo.")
                    self.so.logger.log_state_change(pcb, f"interrupcao_fatal_{interrupt.name}", pcb.state, "FINISHED")
                    if pcb in self.so.gp.ready_queue: self.so.gp.ready_queue.remove(pcb)
                    if pcb in self.so.gp.blocked_queue: self.so.gp.blocked_queue.remove(pcb)
                    self.so.gp.desaloca_processo(pcb)
                    if self.so.hw.cpu.running_process == pcb:
                        self.so.hw.cpu.running_process = None

            # Trata page-fault de forma assíncrona (fora do lock para evitar deadlock)
            if interrupt == Interrupts.PAGE_FAULT:
                thread = threading.Thread(target=self._handle_page_fault_async, args=(pcb, data))
                thread.daemon = True
                thread.start()

    def load_initial_page(self, pcb):
        """Carrega a primeira página do processo na memória"""
        thread = threading.Thread(target=self._load_page_async, args=(pcb, 0))
        thread.daemon = True
        thread.start()

    def _load_page_async(self, pcb, page_idx):
        """Carrega uma página de forma assíncrona"""
        with self.so.gm.lock:
            frame = self.so.gm.find_free_frame()

            if frame == -1:
                # Precisa vitimar uma página
                print(f"    [MEM] Sem frames livres. Iniciando vitimacao para processo {pcb.id}, pagina {page_idx}.")
                frame, victim_pcb, victim_page_entry = self.so.gm.find_victim_frame_fifo()

                # Se a página vítima está suja, salva no disco
                if victim_page_entry and victim_page_entry.dirty_bit:
                    page_size = self.so.gm.get_tam_pg()
                    frame_addr = frame * page_size
                    page_data = [self.so.hw.mem.data[frame_addr + i] for i in range(page_size)]
                    self.so.disk_thread.submit_request("SAVE", victim_pcb, victim_page_entry.page_idx, None, page_data)
                    time.sleep(0.1)  # Aguarda salvamento

        # Submete requisição de carregamento
        self.so.disk_thread.submit_request("LOAD", pcb, page_idx, frame, None)

    def _handle_page_fault_async(self, pcb, logical_address):
        """Trata page-fault de forma assíncrona"""
        page_size = self.so.gm.get_tam_pg()
        page_idx = logical_address // page_size

        print(f"    [PAGE_FAULT] Processo {pcb.id} tentou acessar endereco {logical_address} (pagina {page_idx}).")

        if page_idx >= len(pcb.page_table):
            print(f"    [PAGE_FAULT] ERRO: Pagina {page_idx} fora dos limites para processo {pcb.id}.")
            return

        self._load_page_async(pcb, page_idx)

class Utilities:
    def __init__(self, hw): self.hw = hw
    def dump_word(self, w): print(f"[ {w.opc.name:10s}, R1:{w.ra:2d}, R2:{w.rb:2d}, P:{w.p:4d} ]")
    def dump_memory_range(self, start, end):
        print(f"\n--- DUMP Memoria Fisica (de {start} a {end}) ---")
        for i in range(start, end + 1):
            if 0 <= i < len(self.hw.mem.data):
                print(f"Fis.{i:04d}: ", end="")
                self.dump_word(self.hw.mem.data[i])
        print("--- FIM DUMP ---")

class SO:
    def __init__(self, mem_size, tam_pg, quantum, log_file="system_log.txt"):
        self.quantum, self.shutdown = quantum, False
        self.hw = HW(mem_size, self)
        self.gm = GerenteMemoria(mem_size, tam_pg)
        self.disk = GerenteDisco(self)
        self.logger = SystemLogger(log_file)
        self.gp = GerenteProcessos(self)
        self.utils = Utilities(self.hw)
        self.scheduler = Scheduler(self)
        self.interrupt_handler = InterruptHandling(self)
        self.syscall_handler = SysCallHandling(self)
        self.io_device = IODevice(self)
        self.disk_thread = DiskThread(self)
        self.hw.cpu.set_gerente_memoria(self.gm)

class SysCallHandling:
    def __init__(self, so): self.so = so
    def handle(self):
        pcb = self.so.hw.cpu.running_process
        with self.so.gp.lock:
            pcb.state, pcb.block_reason = PCB.ProcessState.BLOCKED, "I/O Request"
            pcb.pc, pcb.registers = self.so.hw.cpu.pc + 1, self.so.hw.cpu.reg.copy()
            if pcb not in self.so.gp.blocked_queue: self.so.gp.blocked_queue.append(pcb)
            self.so.logger.log_state_change(pcb, "syscall_io", PCB.ProcessState.RUNNING, PCB.ProcessState.BLOCKED)
        self.so.io_device.submit_request(pcb, "WRITE")
        print(f"    [SYSCALL] Processo {pcb.id} bloqueado por I/O.")
    def stop(self):
        pcb = self.so.hw.cpu.running_process
        print(f"    [SYSCALL] STOP no processo {pcb.id}")
        with self.so.gp.lock:
            if pcb is not None:
                pcb.state = PCB.ProcessState.FINISHED
                if pcb in self.so.gp.ready_queue: self.so.gp.ready_queue.remove(pcb)
                if pcb in self.so.gp.blocked_queue: self.so.gp.blocked_queue.remove(pcb)
                self.so.gp.desaloca_processo(pcb)
                self.so.hw.cpu.running_process = None

class Program:
    def __init__(self, name, image): self.name, self.image = name, image

class Programs:
    def __init__(self):
        self.programs = [
            Program("fatorialV2", [
                Word(Opcode.LDI,0,-1,5), Word(Opcode.SYSCALL,-1,-1,-1), Word(Opcode.STD,0,-1,19), 
                Word(Opcode.LDD,0,-1,19), Word(Opcode.LDI,1,-1,-1), Word(Opcode.LDI,2,-1,13), 
                Word(Opcode.JMPIL,2,0,-1), Word(Opcode.LDI,1,-1,1), Word(Opcode.LDI,6,-1,1), 
                Word(Opcode.LDI,7,-1,14), Word(Opcode.JMPIE,7,0,0), Word(Opcode.MULT,1,0,-1), 
                Word(Opcode.SUB,0,6,-1), Word(Opcode.JMP,-1,-1,10), Word(Opcode.STD,1,-1,18), 
                Word(Opcode.STOP,-1,-1,-1), Word(Opcode.DATA,-1,-1,-1), Word(Opcode.DATA,-1,-1,-1)
            ]),
            Program("progMinimo", [
                Word(Opcode.LDI,0,-1,999), Word(Opcode.STD,0,-1,8), Word(Opcode.STOP,-1,-1,-1)
            ]),
            Program("PB", [
                Word(Opcode.LDI,0,-1,7), Word(Opcode.STD,0,-1,50), Word(Opcode.LDD,0,-1,50), 
                Word(Opcode.LDI,1,-1,-1), Word(Opcode.LDI,2,-1,13), Word(Opcode.JMPIL,2,0,-1), 
                Word(Opcode.LDI,1,-1,1), Word(Opcode.LDI,6,-1,1), Word(Opcode.LDI,7,-1,13), 
                Word(Opcode.JMPIE,7,0,0), Word(Opcode.MULT,1,0,-1), Word(Opcode.SUB,0,6,-1), 
                Word(Opcode.JMP,-1,-1,9), Word(Opcode.STD,1,-1,15), Word(Opcode.STOP,-1,-1,-1), 
                Word(Opcode.DATA,-1,-1,-1)
            ]),
            Program("PC", [
                Word(Opcode.LDI,7,-1,5), Word(Opcode.STD,7,-1,45), Word(Opcode.LDI,5,-1,46), 
                Word(Opcode.LDI,0,-1,4), Word(Opcode.STD,0,-1,46), Word(Opcode.LDI,0,-1,3), 
                Word(Opcode.STD,0,-1,47), Word(Opcode.LDI,0,-1,5), Word(Opcode.STD,0,-1,48), 
                Word(Opcode.LDI,0,-1,1), Word(Opcode.STD,0,-1,49), Word(Opcode.LDI,0,-1,2), 
                Word(Opcode.STD,0,-1,50), Word(Opcode.LDI,3,-1,38), Word(Opcode.LDI,6,-1,0), 
                Word(Opcode.LDD,7,-1,45), Word(Opcode.SUBI,6,7,1), Word(Opcode.JMPIE,3,6,0), 
                Word(Opcode.LDX,0,5,-1), Word(Opcode.ADDI,4,5,1), Word(Opcode.LDX,1,4,-1), 
                Word(Opcode.SUB,0,1,0), Word(Opcode.JMPIL,3,0,0), Word(Opcode.STX,5,1,-1), 
                Word(Opcode.STX,4,0,-1), Word(Opcode.ADDI,5,-1,1), Word(Opcode.SUBI,6,-1,1), 
                Word(Opcode.JMPIG,3,6,0), Word(Opcode.SUBI,7,-1,1), Word(Opcode.JMP, -1,-1,14), 
                Word(Opcode.STOP,-1,-1,-1)
            ]),
            Program("fibonacci", [
                Word(Opcode.LDI,0,-1,10), Word(Opcode.SYSCALL,-1,-1,-1), Word(Opcode.STD,0,-1,60),
                Word(Opcode.LDI,1,-1,0), Word(Opcode.LDI,2,-1,1), Word(Opcode.LDI,3,-1,0),
                Word(Opcode.LDD,4,-1,60), Word(Opcode.JMPIE,10,4,0), Word(Opcode.ADD,5,1,2),
                Word(Opcode.MOVE,1,2,-1), Word(Opcode.MOVE,2,5,-1), Word(Opcode.ADDI,3,-1,1),
                Word(Opcode.SUB,4,3,4), Word(Opcode.JMPIG,6,4,0), Word(Opcode.STD,2,-1,61),
                Word(Opcode.SYSCALL,-1,-1,-1), Word(Opcode.STOP,-1,-1,-1), Word(Opcode.DATA,-1,-1,-1),
                Word(Opcode.DATA,-1,-1,-1)
            ]),
            Program("fatorial", [
                Word(Opcode.LDI,0,-1,6), Word(Opcode.SYSCALL,-1,-1,-1), Word(Opcode.STD,0,-1,50),
                Word(Opcode.LDD,0,-1,50), Word(Opcode.LDI,1,-1,1), Word(Opcode.LDI,2,-1,1),
                Word(Opcode.JMPIE,11,0,0), Word(Opcode.MULT,1,0,-1), Word(Opcode.SUBI,0,-1,1),
                Word(Opcode.JMPIG,7,0,0), Word(Opcode.STD,1,-1,51), Word(Opcode.SYSCALL,-1,-1,-1),
                Word(Opcode.STOP,-1,-1,-1), Word(Opcode.DATA,-1,-1,-1), Word(Opcode.DATA,-1,-1,-1)
            ])
        ]
    def retrieve_program(self, name): return next((p for p in self.programs if p.name.lower() == name.lower()), None)

def load_config(config_file="config.ini"):
    """Carrega configurações do arquivo config.ini"""
    config = configparser.ConfigParser()

    # Valores padrão
    defaults = {
        'mem_size': 64,
        'tam_pg': 16,
        'quantum': 5,
        'debug': False,
        'io_latency': 2.0,
        'disk_latency': 0.3,
        'log_file': 'system_log.txt',
        'dump_dir': 'memory_dumps',
        'replacement_policy': 'FIFO'
    }

    if os.path.exists(config_file):
        config.read(config_file)

        if 'MEMORIA' in config:
            defaults['mem_size'] = config.getint('MEMORIA', 'mem_size', fallback=defaults['mem_size'])
            defaults['tam_pg'] = config.getint('MEMORIA', 'tam_pg', fallback=defaults['tam_pg'])

        if 'CPU' in config:
            defaults['quantum'] = config.getint('CPU', 'quantum', fallback=defaults['quantum'])
            defaults['debug'] = config.getboolean('CPU', 'debug', fallback=defaults['debug'])

        if 'IO' in config:
            defaults['io_latency'] = config.getfloat('IO', 'io_latency', fallback=defaults['io_latency'])
            defaults['disk_latency'] = config.getfloat('IO', 'disk_latency', fallback=defaults['disk_latency'])

        if 'LOGGING' in config:
            defaults['log_file'] = config.get('LOGGING', 'log_file', fallback=defaults['log_file'])
            defaults['dump_dir'] = config.get('LOGGING', 'dump_dir', fallback=defaults['dump_dir'])

        if 'PAGINACAO' in config:
            defaults['replacement_policy'] = config.get('PAGINACAO', 'replacement_policy', fallback=defaults['replacement_policy'])

    return defaults

class Sistema:
    def __init__(self, mem_size=None, tam_pg=None, quantum=None, log_file=None, io_latency=None, disk_latency=None, config_file="config.ini"):
        # Carrega configurações do arquivo se existir
        config = load_config(config_file)

        # Usa valores passados como parâmetro ou valores do config
        mem_size = mem_size if mem_size is not None else config['mem_size']
        tam_pg = tam_pg if tam_pg is not None else config['tam_pg']
        quantum = quantum if quantum is not None else config['quantum']
        log_file = log_file if log_file is not None else config['log_file']
        io_latency = io_latency if io_latency is not None else config['io_latency']
        disk_latency = disk_latency if disk_latency is not None else config['disk_latency']

        self.so = SO(mem_size, tam_pg, quantum, log_file)
        self.so.io_device.simulated_io_time = io_latency
        self.so.disk_thread.simulated_disk_time = disk_latency
        self.so.hw.cpu.debug = config['debug']
        self.progs = Programs()

        print(f"[CONFIG] Sistema inicializado com:")
        print(f"  - Memoria: {mem_size} palavras, Paginas: {tam_pg} palavras")
        print(f"  - Quantum: {quantum}, IO Latency: {io_latency}s, Disk Latency: {disk_latency}s")
        print(f"  - Log: {log_file}, Replacement Policy: {config['replacement_policy']}")

    def exibir_estado_completo(self):
        print("\n" + "="*100)
        print("ESTADO COMPLETO DO SISTEMA")
        print("="*100)

        with self.so.gp.lock:
            print(f"\n[PROCESSOS] Total: {len(self.so.gp.all_processes)}")
            for p in self.so.gp.all_processes:
                blocked_reason = f" ({p.block_reason})" if p.state == PCB.ProcessState.BLOCKED else ""
                print(f"  ID: {p.id} | Nome: {p.program.name:<15} | Estado: {p.state.name:<8}{blocked_reason} | PC: {p.pc}")
                print(f"       Tabela de Paginas: {p.get_page_table_string()}")

            print(f"\n[FILA DE PRONTOS] Tamanho: {len(self.so.gp.ready_queue)}")
            for p in self.so.gp.ready_queue:
                print(f"  -> Processo {p.id} ({p.program.name})")

            print(f"\n[FILA DE BLOQUEADOS] Tamanho: {len(self.so.gp.blocked_queue)}")
            for p in self.so.gp.blocked_queue:
                print(f"  -> Processo {p.id} ({p.program.name}) - Razao: {p.block_reason}")

        print(f"\n[CPU] Processo em execucao: ", end="")
        if self.so.hw.cpu.running_process:
            print(f"{self.so.hw.cpu.running_process.id} ({self.so.hw.cpu.running_process.program.name})")
        else:
            print("Nenhum (IDLE)")

        with self.so.gm.lock:
            print(f"\n[MEMORIA] Frames totais: {self.so.gm.num_frames}, Livres: {sum(self.so.gm.free_frames)}, Ocupados: {self.so.gm.num_frames - sum(self.so.gm.free_frames)}")
            print("\nMapa de Frames:")
            for i in range(self.so.gm.num_frames):
                if self.so.gm.free_frames[i]:
                    print(f"  Frame {i:02d}: [LIVRE]")
                else:
                    meta = self.so.gm.frame_metadata[i]
                    print(f"  Frame {i:02d}: [OCUPADO] PID={meta['pid']}, Pagina={meta['page']}, Dirty={meta['dirty']}, LastUsed={meta['last_used']}")

        print(f"\n[DISCO/SWAP] Paginas em swap: {len(self.so.disk.swap_area)}")
        for key in self.so.disk.swap_area.keys():
            print(f"  -> Processo {key[0]}, Pagina {key[1]}")
        print("="*100 + "\n")
    def run_shell(self):
        print("\n============== Sistema Operacional Concorrente ==============")
        print("Comandos: new <prog>, ps, rm <id>, dump <id>, dumpm <ini> <fim>, sysstate, traceon, traceoff, exit")
        while not self.so.shutdown:
            try:
                cmd_line = input("> ").strip().split()
                if not cmd_line: continue
                cmd = cmd_line[0].lower()
                if cmd == "new" and len(cmd_line) > 1:
                    self.so.gp.cria_processo(self.progs.retrieve_program(cmd_line[1]))
                elif cmd == "ps":
                    self.so.gp.list_all_processes()
                elif cmd == "rm" and len(cmd_line) > 1:
                    pcb_id_to_remove = int(cmd_line[1])
                    with self.so.gp.lock:
                        pcb = self.so.gp._find_pcb(pcb_id_to_remove)
                        if pcb:
                            if self.so.hw.cpu.running_process and self.so.hw.cpu.running_process.id == pcb_id_to_remove:
                                print(f"Erro: Nao e possivel remover o processo {pcb_id_to_remove} enquanto esta em execucao.")
                            else:
                                if pcb in self.so.gp.ready_queue: self.so.gp.ready_queue.remove(pcb)
                                if pcb in self.so.gp.blocked_queue: self.so.gp.blocked_queue.remove(pcb)
                                self.so.gp.desaloca_processo(pcb)
                        else:
                            print(f"Processo com ID {pcb_id_to_remove} nao encontrado.")
                elif cmd == "dump" and len(cmd_line) > 1:
                    self.so.gp.dump_processo(int(cmd_line[1]))
                elif cmd == "dumpm" and len(cmd_line) > 2:
                    self.so.utils.dump_memory_range(int(cmd_line[1]), int(cmd_line[2]))
                elif cmd == "sysstate":
                    self.exibir_estado_completo()
                elif cmd == "traceon":
                    self.so.hw.cpu.debug = True
                    print("Modo trace ativado.")
                elif cmd == "traceoff":
                    self.so.hw.cpu.debug = False
                    print("Modo trace desativado.")
                elif cmd == "exit":
                    self.so.shutdown = True
                    self.so.hw.cpu.cpu_has_process_event.set()
                    break
                else:
                    print("Comando invalido ou argumentos faltando.")
            except (ValueError, IndexError):
                print("Erro no comando: Argumento invalido ou ausente.")
            except Exception as e:
                print(f"Erro inesperado: {e}")
    def run(self):
        shell_thread = threading.Thread(target=self.run_shell, name="Shell")
        # Todas as threads de trabalho devem ser daemon para permitir o encerramento limpo.
        shell_thread.daemon = True
        self.so.io_device.daemon = True
        self.so.disk_thread.daemon = True
        self.so.hw.cpu.daemon = True

        self.so.io_device.start()
        self.so.disk_thread.start()
        self.so.hw.cpu.start()
        shell_thread.start()

        try:
            # O thread principal (não-daemon) espera aqui, mantendo o programa vivo.
            # O Ctrl+C será capturado por este bloco.
            while shell_thread.is_alive():
                shell_thread.join(timeout=1.0) # Usa timeout para não bloquear indefinidamente
        except KeyboardInterrupt:
            print("\nCtrl+C detectado. Encerrando o sistema...")

        self.so.shutdown = True
        self.so.hw.cpu.cpu_has_process_event.set() # Acorda a CPU caso ela esteja em wait()
        print("Shell encerrado. Finalizando threads...")

if __name__ == "__main__":
    import sys

    if len(sys.argv) > 1 and sys.argv[1] == "test":
        print("\n" + "="*80)
        print("MODO DE TESTE AUTOMATIZADO")
        print("="*80)

        sistema = Sistema(mem_size=64, tam_pg=16, quantum=5, log_file="test_log.txt", io_latency=1.0, disk_latency=0.2)

        sistema.so.io_device.daemon = True
        sistema.so.disk_thread.daemon = True
        sistema.so.hw.cpu.daemon = True
        sistema.so.io_device.start()
        sistema.so.disk_thread.start()
        sistema.so.hw.cpu.start()

        print("\n[TESTE A] Criando processos Fibonacci e Fatorial com I/O e page-faults...")
        time.sleep(1)

        pid1 = sistema.so.gp.cria_processo(sistema.progs.retrieve_program("fibonacci"))
        time.sleep(1)
        pid2 = sistema.so.gp.cria_processo(sistema.progs.retrieve_program("fatorial"))
        time.sleep(1)

        print("\n[TESTE A] Aguardando execução dos processos (15 segundos)...")
        time.sleep(15)

        print("\n[TESTE A] Estado do sistema após execução:")
        sistema.exibir_estado_completo()

        print("\n[TESTE B] Criando múltiplos processos para forçar vitimação...")
        pid3 = sistema.so.gp.cria_processo(sistema.progs.retrieve_program("PB"))
        time.sleep(1)
        pid4 = sistema.so.gp.cria_processo(sistema.progs.retrieve_program("PC"))
        time.sleep(1)
        pid5 = sistema.so.gp.cria_processo(sistema.progs.retrieve_program("fatorialV2"))
        time.sleep(1)

        print("\n[TESTE B] Aguardando execução e vitimação (20 segundos)...")
        time.sleep(20)

        print("\n[TESTE B] Estado final do sistema:")
        sistema.exibir_estado_completo()

        print("\n[TESTE C] Verificando logs e dumps gerados...")
        if os.path.exists("test_log.txt"):
            print("\n--- CONTEUDO DO LOG (últimas 50 linhas) ---")
            with open("test_log.txt", 'r') as f:
                lines = f.readlines()
                for line in lines[-50:]:
                    print(line.rstrip())

        print("\n[TESTE C] Dumps de memória gerados:")
        if os.path.exists("memory_dumps"):
            for filename in os.listdir("memory_dumps"):
                print(f"  - {filename}")

        print("\n" + "="*80)
        print("TESTES CONCLUÍDOS")
        print("="*80)
        print("\nArquivos gerados:")
        print("  - test_log.txt (log estruturado do sistema)")
        print("  - memory_dumps/ (dumps de memória dos processos finalizados)")
        print("\nPara executar em modo interativo: python sistema_os.py")

        sistema.so.shutdown = True
        sistema.so.hw.cpu.cpu_has_process_event.set()
        time.sleep(2)
    else:
        sistema = Sistema(mem_size=64, tam_pg=16, quantum=5, log_file="system_log.txt", io_latency=2.0, disk_latency=0.3)
        sistema.run()
